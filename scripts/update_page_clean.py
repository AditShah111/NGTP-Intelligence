...
export default function HomePage() {
  ...
  // Clean Workspace Form State (Blank by default)
  ...
  const handleRunEvaluation = async () => {
    ...
    const summary = caseSummary.trim() || `${primaryIssue}. Documents attached: ${uploadedDocuments.length}.`;
  ...
  // Quick Ingest File Drag-and-Drop
  ...
  return (
    <div className="min-h-screen bg-[#FBF9F5] text-slate-900 flex flex-col font-sans">
      ...
      <Header
        ...
        onOpenNewCaseModal={handleResetWorkspace}
      ...
      <main className="flex-grow max-w-5xl w-full mx-auto px-4 sm:px-6 py-6 space-y-6">
        ...
        <div className="bg-white border border-beige-200 rounded-2xl p-6 sm:p-7 shadow-sm">
          <div className="flex items-center justify-between pb-4 border-b border-beige-200">
            ...
            {activeCase ? (
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-amber-50 hover:bg-amber-100 text-amber-900 border border-amber-200 text-xs font-semibold transition-all shadow-sm"
              >
                <FolderOpen className="w-3.5 h-3.5 text-amber-700" />
                <span>View Historical Matters ({cases.length})</span>
              </button>
            )}
          </div>
          ...
          {/* Quick Evidence Dropzone */}
          <div className="mt-5 pt-4 border-t border-beige-200">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-2">
              <span className="text-xs font-semibold text-slate-800 flex items-center gap-1.5">
                <Upload className="w-3.5 h-3.5 text-amber-700" />
                <span>Upload Evidence Documents (Invoices, Bank Statements, E-Way bills):</span>
              </span>
          ...
          {/* Action Button */}
          <div className="mt-5 pt-4 border-t border-beige-200 flex items-center justify-between">
            <span className="text-xs text-slate-500">
              {uploadedDocuments.length === 0 
                ? 'Tip: Upload bank statements and invoices for an accurate evidentiary audit.' 
                : `${uploadedDocuments.length} document(s) ready for background statutory audit.`}
            </span>

            <button
              onClick={handleRunEvaluation}
              disabled={isLoading}
              className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-amber-600 hover:bg-amber-700 disabled:opacity-50 text-white font-semibold text-xs shadow-md transition-all whitespace-nowrap"
            >
              <Zap className="w-4 h-4 fill-white" />
              <span>{isLoading ? 'Running Intelligence Agents...' : 'Run Legal Intelligence Engine'}</span>
            </button>
          </div>
        </div>
