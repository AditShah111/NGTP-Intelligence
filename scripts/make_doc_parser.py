...
code_parser = """export interface ExtractedDocumentData {
  name: string;
  type: string;
  textSnippet: string;
  hasInvoices: boolean;
...
with open("src/service/document-parser.ts", "w", encoding="utf-8") as f:
    f.write(code_parser)

print("Created src/service/document-parser.ts")