...
import { CaseStudy, CaseDocument, PrecedentAnalysis } from '../types';
...
export default function HomePage() {
  ...
  // Continuous Precedent Ingestion and Dynamic Parameter Auto-Calibration
  const handleIngestPrecedents = async (domain: string, customQuery?: string) => {
    if (!activeCase) return;
    setIsIngesting(true);
    try {
      const geminiApiKey = typeof window !== 'undefined' ? (localStorage.getItem('ngtp_gemini_api_key') || undefined) : undefined;
      const res = await fetch('/api/ingest-precedents', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          topicDomain: domain,
          ...
          existingPrecedents: activeCase.precedents
        })
      });

      if (res.ok) {
        const data = await res.json();
        if (data.ingestedPrecedents) {
          const updatedCase: CaseStudy = {
            ...activeCase,
            precedents: data.ingestedPrecedents,
            statutoryParameters: data.calibratedParameters || activeCase.statutoryParameters
          };
      ...
      console.error('Precedent Ingestion error:', err);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#FBF9F5] text-slate-900 flex flex-col font-sans">
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-4 sm:px-6 py-6">
        ...
        {isIngesting && (
          <div className="mb-4 p-3.5 rounded-xl bg-purple-50 border border-purple-200 flex items-center justify-between text-xs text-purple-900 font-medium animate-pulse shadow-sm">
            <div className="flex items-center gap-2">
              <Sparkles className="w-4 h-4 text-purple-700 animate-spin" />
              <span>Ingesting live judicial precedents, analyzing Article 141 hierarchy, and calibrating parameters...</span>
            </div>
            <span className="font-mono text-[11px] font-semibold text-purple-800">Calibrating Parameters...</span>
          </div>
        )}
        ...
        {activeCase ? (
          <div className="animate-fade-in">
            ...
            <div className="transition-all duration-200">
              ...
              {activeStep === 3 && (
                <Step3Step4PrecedentsView 
                  precedents={activeCase.precedents} 
                  onIngestPrecedents={handleIngestPrecedents}
