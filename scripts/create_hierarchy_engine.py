...
export function calculateJudicialAuthorityStrength(
  ...
  if (slpStatus === 'Affirmed by Supreme Court') {
    ...
    score = Math.min(score, 45); // Unreliable authority if stayed
  ...
  return precedents.map(p => {
    ...
    if (isSuncraft) {
      ...
      conflictAnalysis = {
        ...
        whyThisPrecedentPrevails: 'Arise India establishes the constitutional doctrine of impossibility (Article 14) which was affirmed by the Supreme Court in SLP (C) No. 36717/2017. A bona fide buyer cannot be penalized for default of a registered seller beyond the buyer\'s control.',
...
print("Created src/service/hierarchy-engine.ts with Article 141 judicial hierarchy & conflict resolution!")