        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-start">
            <div 
              ...
              title="Return to clean workspace"
            >
          ...
          <div className="flex flex-wrap items-center gap-2 w-full md:w-auto justify-end">
            {/* Clean Workspace / New Assessment */}
            <button
              onClick={onResetToCleanWorkspace}
              className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg font-semibold transition-all shadow-sm ${
                activeCase 
                  ? 'bg-amber-600 hover:bg-amber-700 text-white' 
                  : 'bg-beige-100 text-slate-800 border border-beige-300'
              }`}
              title="Start a fresh, clean assessment"
            >
              <PlusCircle className="w-3.5 h-3.5" />
              <span>{activeCase ? '✕ Clear Workspace' : 'Clean Workspace'}</span>
            </button>

            {/* Historical Matters Drawer Trigger (Desktop) */}
            <button
              onClick={onOpenHistoricalDrawer}
              className="hidden md:flex items-center gap-1.5 text-xs bg-white hover:bg-beige-50 text-slate-700 border border-beige-300 px-3 py-1.5 rounded-lg transition-all font-medium shadow-sm"
              title="View past assessments and cached cases"
            >
              <FolderOpen className="w-3.5 h-3.5 text-amber-700" />
              <span>Historical Matters ({cases.length})</span>
            </button>

            {/* Gemini API Key Toggle */}
            <button
              onClick={() => setIsKeyModalOpen(true)}
              className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-lg border transition-all font-medium shadow-sm ${
                hasApiKey 
            ...
            {activeCase && (
              <button
                ...
                className="flex items-center gap-1.5 text-xs bg-slate-900 hover:bg-slate-800 text-white font-semibold px-3 py-1.5 rounded-lg transition-all shadow-sm"
              >
            ...
            <div className="flex items-center gap-1 text-[11px] font-mono px-2 py-1 rounded-lg bg-beige-100/80 border border-beige-200 text-slate-600">
