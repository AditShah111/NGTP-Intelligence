...
case1 = {
  ...
  "disputedAmount": "INR 48,50,000 (ITC) + Penalty u/s 73 + Interest",
  "noticeType": "Order-in-Original / DRC-07",
  "primaryIssue": "Disallowance of ITC under Section 16(2)(c) due to supplier (M/s Steel Corp) filing GSTR-1 but defaulting in GSTR-3B, without prior investigation against supplier.",
  ...
  "documents": [
    {
      ...
      "extractedTextSnippet": "Supplier M/s Steel Corp failed to remit tax in GSTR-3B. Hence condition of Section 16(2)(c) is not satisfied. ITC disallowed with equal penalty."
    },
    {
      "id": "doc-2",
  ...
  "statutoryParameters": [
    {
      ...
      "availableEvidence": ["14 Tax Invoices on record", "Rule 46 compliance audit certificate"],
    ...
    },
    {
      ...
      "requiredEvidence": ["E-Way Bills", "Lorry Receipts (LR)", "Gate Inward Registers", "Weighbridge Slips"],
      "availableEvidence": ["E-Way bills (Part A & B)", "Store Inward Vouchers", "Stock ledger consumption records"],
      "assessment": "SATISFIED",
      "risk": "LOW",
    ...
    },
    {
      ...
      "requiredEvidence": ["DRC-01A / DRC-01 issued to supplier", "Attachment proceedings against supplier"],
      "availableEvidence": ["Supplier filed GSTR-1 (tax invoiced)", "Zero recovery steps taken by Department against supplier"],
    ...
    },
    {
      ...
      "availableEvidence": ["Section 16(2)(aa) inserted w.e.f 01.01.2022 and is strictly prospective"],
      "assessment": "SATISFIED",
      "risk": "LOW",
  ...
  "precedents": [
    {
      ...
      "distinguishingFacts": ["None. Facts are on all fours."],
      ...
      "presentCaseEvidenceSatisfying": ["Invoices", "RTGS payment slips", "GSTR-1 upload extract", "DRC-07 admission of no action on seller"],
    ...
    },
    {
      ...
      "materialFacts": "Sellers omitted to pay tax despite collecting from purchaser. Revenue issued demand to buyers without examining sellers.",
      "ratioLegalPrinciple": "When the sellers have collected tax, the Proper Officer is bound to initiate recovery against the sellers and must examine them in inquiry before burdening the purchasing dealer.",
      "necessaryConditions": ["Tax collected by supplier in invoice", "Buyer paid consideration including tax through bank"],
  ...
  "lowerAuthorityErrors": [
    ...
    {
      "id": "err-2",
      "finding": "Equal Penalty of INR 48,50,000 imposed under Section 73(9).",
      "lowerAuthorityReasoning": "Non-payment of tax by supplier deemed to be incorrect availment by buyer.",
      "evidenceIgnoredMisread": "Ignored lack of any mens rea, suppression, or misstatement by the bona fide taxpayer.",
      "legalError": "Section 73(9) penalty cannot be levied mechanically when tax was paid in good faith under existing returns.",
  ...
  "redTeamItems": [
    ...
    {
      "id": "rt-2",
      "category": "Burden of Proof u/s 155",
      "opposingArgument": "Under Section 155, the burden of proving that ITC has been lawfully availed lies squarely on the person claiming it. Producing an invoice and bank statement is insufficient to prove that government received the tax.",
      "strengthOfOpposingArgument": 75,
      "taxpayerResponse": "The taxpayer discharged the initial burden by proving invoice, movement of goods, and payment of tax to supplier. The burden to prove remittance to treasury shifts to the Revenue who possesses exclusive access to the supplier's GSTR-3B cash ledgers.",
  ...
  "forwardDecision": {
    ...
    "scoreReducers": ["Failure to tender proof of supplier active registration at the time of transaction"],
    "evidenceDependentImprovements": ["Supplier VAT/GST active status certificate from GST portal on invoice dates"],
    "nonCurableWeaknesses": ["None identified in present case record"],
  ...
  "draftAudit": [
    ...
    {
      "id": "da-2",
      "parameter": "Circular 183 Safe-Harbor",
      "issueDetected": "Pleadings do not explicitly invoke CBIC Circular No. 183/15/2022-GST.",
  ...
  "finalOutput": {
    ...
    "precedentMatrix": [
      { "precedent": "Suncraft Energy (Cal HC / SC)", "applicability": "Controlling / Direct Ratio", "score": 100 },
      { "precedent": "D.Y. Beathel Enterprises (Mad HC)", "applicability": "Direct Ratio on seller recovery", "score": 96 },
      { "precedent": "Arise India Ltd. (Del HC / SC)", "applicability": "Constitutional doctrine of impossibility", "score": 92 }
    ],
    "lowerAuthorityErrorMatrix": [
      { "error": "Disallowance solely based on GSTR-2A mismatch without examining seller", "significance": "Fundamental jurisdictional defect" },
      { "error": "Mechanical imposition of maximum Section 73 penalty", "significance": "Serious legal error" }
    ],
    "draftDefects": [
      { "defect": "Missing Supreme Court SLP citation for Suncraft Energy", "severity": "Medium" },
      { "defect": "Omission of explicit ground on Circular 183/15/2022 safe harbor", "severity": "High" }
    ],
...
benchmarks_ts = f"import {{ CaseStudy }} from '../types';\n\nexport const BENCHMARK_CASES: CaseStudy[] = [\n{json.dumps(case1, indent=2)}\n];\n"
write_file("src/repo/benchmark-data.ts", benchmarks_ts)

case_repo_ts = """import { CaseStudy } from '../types';
...
"""
write_file("src/repo/case-repo.ts", case_repo_ts)

print("Repo files generated successfully!")