...
export type BenchType = 
  | 'Supreme Court Full/Constitution Bench' 
  | 'Supreme Court Division Bench' 
  | 'High Court Full Bench' 
  | 'High Court Division Bench' 
  | 'High Court Single Bench' 
  | 'Appellate Tribunal (CESTAT / GSTAT)';

export type Article141Precedence = 
  | 'SUPREME_BINDING' 
  | 'HIGH_COURT_BINDING' 
  | 'PERSUASIVE' 
  | 'OVERRULED' 
...
export interface StatutoryParameter {
  ...
  risk: RiskLevel;
  reason: string;
  dynamicWeightModifier?: number;
  courtEvidentiaryPrecedent?: string;
  controllingAuthorityLevel?: BenchType;
  article141Status?: Article141Precedence;
}

export interface PrecedentEvidentiaryImpact {
  parameterCode: string;
  impactDescription: string;
  weightModifier: number;
}

export interface CompetingConflictAnalysis {
  conflictWith: string;
...
export interface PrecedentAnalysis {
  ...
  evidentiaryWeightImpact?: PrecedentEvidentiaryImpact[];
  
  // Judicial Hierarchy & Precedence Metrics
  benchType?: BenchType;
  benchStrength?: number;
  article141Status?: Article141Precedence;
  slpStatus?: SlpStatus;
  judicialAuthorityStrengthScore?: number; // 0-100
  competingConflictAnalysis?: CompetingConflictAnalysis;
