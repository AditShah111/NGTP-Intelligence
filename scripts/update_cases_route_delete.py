  ...
  try {
    ...
    if (id) {
      ...
      return NextResponse.json({ 
        success: deleted, 
        message: deleted ? `Case ${id} permanently deleted` : `Failed to delete case ${id}` 
      });
    } else {
      // Clear all historical cases
      const cleared = await clearAllCases();
      return NextResponse.json({ 
        success: cleared, 
        message: 'All historical cases permanently deleted from database and cache.' 
      });
    }
  } catch (err: any) {
