...
export async function runComplete13StepEvaluation(
  ...
  if (geminiApiKey || process.env.GEMINI_API_KEY) {
    try {
      const livePrecedents = await fetchLatestPrecedentsWithGemini(primaryIssue, financialYear, geminiApiKey);
  ...
  if (readinessScore.totalScore < 40) {
    top5Reasons = [
      ...
      'Suncraft Energy and D.Y. Beathel protections CANNOT be invoked without demonstrating genuine banking payments.',
  ...
  const finalOutput: FinalEvaluatorOutput = {
    ...
    precedentMatrix: precedents.map(p => ({
      ...
      applicability: hasBank ? (p.favourableApplicability === 'HIGH' ? 'Controlling / Direct Ratio' : 'Persuasive') : 'Inapplicable (Bank Payment Unproven)',
...
print("Updated evaluator-agent.ts with strict document type gating!")