...
export async function getAllCases(): Promise<CaseStudy[]> {
  try {
    ...
    console.warn('⚠️ [CaseRepo] DB read fallback to memory:', err.message);
  }
...
export async function getCaseById(id: string): Promise<CaseStudy | null> {
  ...
  try {
    ...
    console.warn('⚠️ [CaseRepo] DB read error for ID:', id, err.message);
  }
  return null;
}
...
export async function saveCase(caseData: CaseStudy): Promise<CaseStudy> {
  ...
  try {
    ...
    console.warn('⚠️ [CaseRepo] DB save error, stored in memory cache:', err.message);
  }
  return caseData;
}
...
export async function deleteCaseById(id: string): Promise<boolean> {
  ...
  try {
    const isDbReady = await initDbSchema();
    if (isDbReady) {
      await pool.query('DELETE FROM ngtp.cases WHERE id = $1', [id]);
      await pool.query('DELETE FROM ngtp.audit_logs WHERE case_id = $1', [id]);
    }
    return true;
  } catch (err: any) {
    console.warn('⚠️ [CaseRepo] DB delete error:', err.message);
    return false;
  }
}
...
export async function clearAllCases(): Promise<boolean> {
  ...
  try {
    const isDbReady = await initDbSchema();
    if (isDbReady) {
      await pool.query('DELETE FROM ngtp.cases;');
      await pool.query('DELETE FROM ngtp.audit_logs;');
      console.log('✓ [CaseRepo] Permanently cleared all cases and audit logs from database.');
    }
    return true;
  } catch (err: any) {
    console.warn('⚠️ [CaseRepo] DB clear error:', err.message);
    return false;
  }
}
